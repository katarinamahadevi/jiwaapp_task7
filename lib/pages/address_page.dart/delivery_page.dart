...
class DeliveryPage extends GetView<AddressController> {
  ...
  const DeliveryPage({Key? key, this.isSelectionMode = false})
    : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppbarPrimary(
        title: isSelectionMode ? 'Pilih Alamat' : 'Alamat Tersimpan',
      ),
      body: Column(
        children: [
  ...
  Widget _buildAddressItem({
    ...
    return GestureDetector(
      onTap:
          isSelectionMode
              ? () {
                // MODIFIED: Select address and return with address_id
                controller.selectAddress(address.id);
                print('Address selected in DeliveryPage: ${address.id}');

                // Return the selected address data to OrderController
                Get.back(
                  result: {
                ...
                child: Column(
                  children: [
                    ...
                    if (!isHighlighted) ...[
                      ...
                      GestureDetector(
                        onTap: () {
                          // MODIFIED: For non-selection mode, just select the address
                          controller.selectAddress(address.id);
                          print(
                            'Address selected (non-selection mode): ${address.id}',
                          );
                        },
                        child: Container(
                          width: 25,
